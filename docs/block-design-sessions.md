...
During the discussion, you can speak normally. When `workflow.refine` is called with the block id, ConstantX automatically creates or updates `annotation-<BLOCK_ID>.md` and `design-session.md`. Codex should compare your requested changes against the pins, block scope, extracted evidence, existing directives, and the original plan, then record the substantive decision through the same refine workflow.
...
The generated `spec.md` must cite finalized design-session `[n]` checkpoints when present, approved directives, implementation target, evidence/model fit, exact files/artifacts to create or modify, artifacts to remove or replace, non-goals, acceptance criteria, verification plan, traceability, and a Non-Minimal Implementation Requirement that forbids stub, placeholder, toy, demo-only, minimal, superficial, TODO-driven, or partial implementations.
