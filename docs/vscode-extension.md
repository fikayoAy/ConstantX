...
The ConstantX VS Code extension is the admin and visibility layer for the prompt-first MCP workflow. In `0.1.4`, it starts and supervises the shared local HTTP MCP runtime automatically, using the bundled server inside the extension by default.
...
%LOCALAPPDATA%\ConstantX\server\0.1.4\
```
...
```text
dist-vsix/constantx-vscode-0.1.4.vsix
```
